// updated Editor.tsx with ArticleReadComponent and preview tab
        ...
        <Card className="p-4 space-y-4 w-full max-w-3xl">
          <Tabs value={tab} onValueChange={setTab} className="w-full">
            ...
            <TabsContent value="card">
              <motion.div key="card" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded border border-border p-2 dark:bg-black bg-white">
                <ArticleCard Heading={articleHeading} date={articleDate} imgUrl={imgUrl} />
              </motion.div>
            </TabsContent>
            <TabsContent value="read">
              <motion.div key="read" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="rounded border border-border p-2 dark:bg-black bg-white">
              ...
              <Tabs value={tabValue} onValueChange={setTabValue} className="px-4">
                ...
                {tabValue === "link" && (
                  <TabsContent value="link">
                    ...
                    <Button onClick={handleImgUrlSet} className="mt-2">Set</Button>
                    {imgUrlError && <p className="text-sm text-red-500">{imgUrlError}</p>}
                  </TabsContent>
                )}
                {tabValue === "file" && (
                  <TabsContent value="file">
                    ...
                    {imgUrl && (
                      <img src={imgUrl} alt="preview" className="mt-4 rounded" />
                    )}
                  </TabsContent>
                )}
              </Tabs>
